...
***REMOVED***0 => array('Unknown direction specified in call to StrokeBoxedText() [%s]',1),
***REMOVED***1 => array('Internal font does not support drawing text at arbitrary angle. Use TTF fonts instead.',0),
***REMOVED***2 => array('There is either a configuration problem with TrueType or a problem reading font file "%s" Make sure file exists and is in a readable place for the HTTP process. (If \'basedir\' restriction is enabled in PHP then the font file must be located in the document root.). It might also be a wrongly installed FreeType library. Try upgrading to at least FreeType 2.1.13 and recompile GD with the correct setup so it can find the new FT library.',1),
***REMOVED***3 => array('Can not read font file "%s" in call to Image::GetBBoxTTF. Please make sure that you have set a font before calling this method and that the font is installed in the TTF directory.',1),
***REMOVED***4 => array('Direction for text most be given as an angle between 0 and 90.',0),
***REMOVED***5 => array('Unknown font font family specification. ',0),
***REMOVED***6 => array('Can\'t allocate any more colors in palette image. Image has already allocated maximum of %d colors and the palette  is now full. Change to a truecolor image instead',0),
***REMOVED***7 => array('Color specified as empty string in PushColor().',0),
***REMOVED***8 => array('Negative Color stack index. Unmatched call to PopColor()',0),
***REMOVED***9 => array('Parameters for brightness and Contrast out of range [-1,1]',0),
