...
***REMOVED***0 => array('Unbekannte Richtung angegeben im Aufruf von StrokeBoxedText() [%s].',1),
***REMOVED***1 => array('Die interne Schrift untestützt das Schreiben von Text in einem beliebigen Winkel nicht. Benutze stattdessen TTF-Schriften.',0),
***REMOVED***2 => array('Es liegt entweder ein Konfigurationsproblem mit TrueType oder ein Problem beim Lesen der Schriftdatei "%s" vor. Versichere Dich, dass die Datei existiert und Leserechte und -pfad vergeben sind. (wenn \'basedir\' restriction in PHP aktiviert ist, muss die Schriftdatei im Dokumentwurzelverzeichnis abgelegt werden). Möglicherweise ist die FreeType-Bibliothek falsch installiert. Versuche, mindestens zur FreeType-Version 2.1.13 zu aktualisieren und kompiliere GD mit einem korrekten Setup neu, damit die FreeType-Bibliothek gefunden werden kann.',1),
***REMOVED***3 => array('Die Schriftdatei "%s" kann nicht gelesen werden beim Aufruf von Image::GetBBoxTTF. Bitte versichere Dich, dass die Schrift gesetzt wurde, bevor diese Methode aufgerufen wird, und dass die Schrift im TTF-Verzeichnis installiert ist.',1),
***REMOVED***4 => array('Die Textrichtung muss in einem Winkel zwischen 0 und 90 engegeben werden.',0),
***REMOVED***5 => array('Unbekannte Schriftfamilien-Definition. ',0),
***REMOVED***6 => array('Der Farbpalette können keine weiteren Farben zugewiesen werden. Dem Bild wurde bereits die größtmögliche Anzahl von Farben (%d) zugewiesen und die Palette ist voll. Verwende stattdessen ein TrueColor-Bild',0),
***REMOVED***7 => array('Eine Farbe wurde als leerer String im Aufruf von PushColor() angegegeben.',0),
***REMOVED***8 => array('Negativer Farbindex. Unpassender Aufruf von PopColor().',0),
***REMOVED***9 => array('Die Parameter für Helligkeit und Kontrast sind außerhalb des zulässigen Bereichs [-1,1]',0),
