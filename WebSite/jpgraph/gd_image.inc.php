...
//                JpGraphError::RaiseL(25087);//('This PHP build has not been configured with TTF support. You need to recompile your PHP installation with FreeType support.');
    ...
    function StrokeBoxedText($x,$y,$txt,$dir=0,$fcolor="white",$bcolor="black",
        ...
        if( !is_numeric($dir) ) {
            if( $dir=="h" ) $dir=0;
            elseif( $dir=="v" ) $dir=90;
            else JpGraphError::RaiseL(***REMOVED***0,$dir);//(" Unknown direction specified in call to StrokeBoxedText() [$dir]");
        }
    ...
    function _StrokeBuiltinFont($x,$y,$txt,$dir,$paragraph_align,&$aBoundingBox,$aDebug=false) {
        ...
        JpGraphError::RaiseL(***REMOVED***1);//(" Internal font does not support drawing text at arbitrary angle. Use TTF fonts instead.");
    ...
    function imagettfbbox_fixed($size, $angle, $fontfile, $text) {
        ...
        if( ! USE_LIBRARY_IMAGETTFBBOX ) {
            ...
            if( $bbox === false ) {
                JpGraphError::RaiseL(***REMOVED***2,$this->font_file);
    ...
    function GetBBoxTTF($aTxt,$aAngle=0) {
        ...
        if( !is_readable($this->font_file) ) {
            JpGraphError::RaiseL(***REMOVED***3,$this->font_file);
    ...
    function StrokeText($x,$y,$txt,$dir=0,$paragraph_align="left",$debug=false) {
        ...
        if( !is_numeric($dir) ) {
            JpGraphError::RaiseL(***REMOVED***4);//(" Direction for text most be given as an angle between 0 and 90.");
        }
        ...
        elseif( $this->font_family >= _FIRST_FONT && $this->font_family <= _LAST_FONT)  {
            ...
            JpGraphError::RaiseL(***REMOVED***5);//(" Unknown font font family specification. ");
        }
        return $boundingbox;
    }
    ...
    function PushColor($color) {
        if( $color != "" ) {
            ...
            JpGraphError::RaiseL(***REMOVED***7);//("Color specified as empty string in PushColor().");
        }
    }

    function PopColor() {
        if( $this->colorstackidx < 1 ) {
            JpGraphError::RaiseL(***REMOVED***8);//(" Negative Color stack index. Unmatched call to PopColor()");
        }
